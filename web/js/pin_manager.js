...
class PinManager {
	...
	renderRows(highlightJoint) {
		...
		const jointLabels = {
			...
			ombro_master: "Ombro Master",
			ombro_slave: "Ombro Slave Invertido",
			cotovelo: "Cotovelo",
			punho: "Punho",
	...
	async save() {
		...
		// Sincroniza imediatamente com o LimitsManager e com o painel lateral de ângulos
		if (window.limitsManager) {
			window.limitsManager.updatePinMapping(updated)
		}
		if (window.updateRightPanelPinLabels) {
			window.updateRightPanelPinLabels(updated)
		}
		...
		try {
			...
			if (data.success) {
				this.close()
				if (window.showToast) window.showToast("Mapeamento de pinos atualizado e sincronizado!")
			}
		} catch (e) {
	...
	async fetchInitialPins() {
		try {
			const res = await fetch("/api/pins")
			if (res.ok) {
				const data = await res.json()
				if (data && data.pins) {
					this.currentMapping = data.pins
					if (window.limitsManager) {
						window.limitsManager.updatePinMapping(data.pins)
					}
					if (window.updateRightPanelPinLabels) {
			...
			console.warn("Não foi possível carregar pinos iniciais:", e)
		}
	}
}
